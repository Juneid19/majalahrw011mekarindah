...
var catCf={keamanan:{l:'Keamanan',c:'#ef4444'},pertamanan:{l:'Pertamanan',c:'#22c55e'},haribesar:{l:'Hari Besar',c:'#f43f5e'},sosial:{l:'Sosial Warga',c:'#3b82f6'},pkk:{l:'PKK',c:'#a855f7'},olahraga:{l:'Olahraga',c:'#f97316'},hukum:{l:'Hukum',c:'#6366f1'},ekonomi:{l:'Ekonomi',c:'#f59e0b'},kesehatan:{l:'Kesehatan',c:'#10b981'},sampah:{l:'Sampah',c:'#14b8a6'},komunikasi:{l:'Komunikasi',c:'#06b6d4'}};
var sK=['keamanan','pertamanan','haribesar','sosial','pkk','olahraga','hukum','ekonomi','kesehatan','sampah','komunikasi'];
var sL=['Keamanan','Pertamanan','Hari Besar','Sosial Warga','PKK','Olahraga','Hukum','Ekonomi','Kesehatan','Sampah','Komunikasi'];
...
function popStr(){var f=function(iid,val){var el=document.getElementById(iid);if(el)el.value=val||'';};f('st_wakil',D_ST.wakil);f('st_fw',D_ST.fotoWakil);f('st_sek',D_ST.sek);f('st_fse',D_ST.fotoSek);f('st_ben',D_ST.ben);f('st_fbe',D_ST.fotoBen);for(var i=0;i<11;i++)f('st_s'+(i+1),D_ST.s[i]||'');for(i=0;i<6;i++){f('st_r'+(i+1),D_ST.r[i]||'');f('st_fr'+(i+1),D_ST.fotoR&&D_ST.fotoR[i]?D_ST.fotoR[i]:'');}}
...
function popStr(){
    try{
        ...
        for(var i=0;i<11;i++){var sEl=document.getElementById('st_s'+(i+1));if(sEl)sEl.value=D_ST.s[i]||'';var fsEl=document.getElementById('st_fs'+(i+1));if(fsEl)fsEl.value=(D_ST.fotoS&&D_ST.fotoS[i])?D_ST.fotoS[i]:'';}
...
function svStr(){
    try{
        ...
        D_ST.s=[];D_ST.fotoS=[];for(var i=0;i<11;i++){var sEl=document.getElementById('st_s'+(i+1));D_ST.s.push(sEl?sEl.value.trim():'');var fsEl=document.getElementById('st_fs'+(i+1));D_ST.fotoS.push(fsEl?fsEl.value.trim():'');}
