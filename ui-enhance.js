...
function turboLazyImages(){
    try {
        document.querySelectorAll('img:not([loading]):not([src*="data:"])').forEach(function(img){
...
/* === JOKER CARD: AUTO CONVERT CLOUDINARY LINK KE IFRAME === */
function fixCloudinaryEmbeds(){
    try {
        document.querySelectorAll('a[href*="player.cloudinary.com"], video[src*="player.cloudinary.com"], source[src*="player.cloudinary.com"]').forEach(function(el){
            var url = el.href || el.src;
            var iframe = document.createElement('iframe');
            iframe.src = url;
            iframe.style.cssText = 'width:100%;aspect-ratio:16/9;border:none;border-radius:1rem;box-shadow:0 0 20px rgba(0,0,0,.3);';
...
function forcePeraturan(){
    try {
        ...
        if(!document.getElementById('sec-peraturan')){
            ...
            sec.className = 'sec sec-pad';
            sec.id = 'sec-peraturan';
            ...
            var nextEl = mading.nextSibling;
            mading.parentNode.insertBefore(secDiv, nextEl);
            mading.parentNode.insertBefore(sec, nextEl);
        }

        var peraturan = document.getElementById('sec-peraturan');
        var pElem = peraturan.querySelector('p.pi');
        var hElem = peraturan.querySelector('h2');
...
function initGaleriCaption(){
    ...
    captionWrap.id = 'custom-caption-wrap';
    ...
    function updateCaption(){
        ...
        if(!track || !captionWrap) return;
        var transform = track.style.transform || '';
        var match = transform.match(/translateX\(-?(\d+)%\)/);
        if(match){
            var percent = parseInt(match[1]);
            var index = Math.round(percent / 100);
            var captions = track.querySelectorAll('.slide-item>div[style*="linear-gradient"]');
            captionWrap.textContent = (captions[index] && captions[index].textContent) ? captions[index].textContent.trim() : '';
        } else {
            var caps = track.querySelectorAll('.slide-item>div[style*="linear-gradient"]');
            captionWrap.textContent = (caps[0] && caps[0].textContent) ? caps[0].textContent.trim() : '';
        }
    }
    ...
    var track = document.getElementById('galeriTrack');
    if(track){
        new MutationObserver(updateCaption).observe(track, { attributes: true, attributeFilter: ['style'] });
    }
}
...
function watchMagV(target){
    new MutationObserver(function(){
        ...
        safeAct();
        turboLazyImages(); 
        fixCloudinaryEmbeds(); // Panggil Joker Card
...
if(mv) {
    ...
    var tempCctv = new MutationObserver(function(){
        if(document.getElementById('magV')) {
            this.disconnect();
            watchMagV(document.getElementById('magV'));
        }
